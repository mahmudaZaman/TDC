...
passengers = pd.DataFrame(df["#Passengers"]).rename(columns={"#Passengers": "total_passengers"})
print(passengers.head())

# Get bitcoin date array
timesteps = passengers.index.to_numpy()
total_passengers = passengers["total_passengers"].to_numpy()
print(timesteps[:10], total_passengers[:10])

HORIZON = 1 # predict 1 step at a time
WINDOW_SIZE = 7 # use a week worth of timesteps to predict the horizon

full_windows, full_labels = make_windows(total_passengers, window_size=WINDOW_SIZE, horizon=HORIZON)
