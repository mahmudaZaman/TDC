...
df = pd.read_csv("dataset/daily_minimum_temps.csv",
                 parse_dates=["Date"],
                 index_col=["Date"]) # parse the date column (tell pandas column 1 is a datetime)
print(df.head())
print(df.info())

temperatures = pd.DataFrame(df["Temp"]).rename(columns={"Temp": "temperature"})
df['Temp'] = df['Temp'].astype(float)
print(temperatures.head())

# Get bitcoin date array
timesteps = temperatures.index.to_numpy()
temperature = temperatures["temperature"].to_numpy()
print(timesteps[:10], temperature[:10])

HORIZON = 1 # predict 1 step at a time
WINDOW_SIZE = 7 # use a week worth of timesteps to predict the horizon

full_windows, full_labels = make_windows(temperature, window_size=WINDOW_SIZE, horizon=HORIZON)
