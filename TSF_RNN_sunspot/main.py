...
df = pd.read_csv("dataset/sunspots.csv",
                 parse_dates=["Date"],
...
sunspots = pd.DataFrame(df["Monthly Mean Total Sunspot Number"]).rename(columns={"Monthly Mean Total Sunspot Number": "sunspot"})
print(sunspots.head())

timesteps = sunspots.index.to_numpy()
sunspot = sunspots["sunspot"].to_numpy()
print(timesteps[:10], sunspot[:10])

HORIZON = 1 # predict 1 step at a time
WINDOW_SIZE = 7 # use a week worth of timesteps to predict the horizon

full_windows, full_labels = make_windows(sunspot, window_size=WINDOW_SIZE, horizon=HORIZON)
